...
def index():
    ...
    pdf_files = [file for file in os.listdir(upload_folder) if allowed_file(file)]
...
def search():
    ...
    pdf_files = [
        ...
        if allowed_file(file) and query.lower() in file.lower()
    ]
    if len(pdf_files) > 20:
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        flash(f"Only these file extensions {ALLOWED_EXTENSIONS} are allowed")
        error_message = ("An error occurred while processing your request. File Extension not allowed. ")
        return render_template('error.html', error_message=error_message)
